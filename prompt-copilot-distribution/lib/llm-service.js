// LLM Service for AI-powered prompt refinement using OpenAI API
...
class LLMService {
  constructor() {
    ...
    this.models = {
      fast: 'gpt-4o-mini',  // Fast, cost-effective
      expert: 'gpt-4o'  // Advanced reasoning model
    };
    this.apiKey = null;
  ...
  /**
   * Validate OpenAI API key format (starts with 'sk-' and is approximately 51 characters)
   */
  validateApiKey(apiKey) {
    ...
    // Allow some flexibility for different key types (sk-proj-, sk-, etc.)
    return apiKey.trim().startsWith('sk-') && apiKey.trim().length >= 20;
  }

  async init() {
    // Load API key and mode from storage
    const result = await chrome.storage.local.get(['openaiApiKey', 'aiMode']);
  ...
  getModelInfo() {
    if (this.mode === 'expert') {
      return { name: 'GPT-4o', description: 'Advanced reasoning model' };
    }
    return { name: 'GPT-4o Mini', description: 'Fast refinement' };
  }

  /**
   * Extract final answer from model output (removes any thinking process if present)
   */
  extractFinalAnswer(text) {
    // Remove thinking tags and content between them if present
