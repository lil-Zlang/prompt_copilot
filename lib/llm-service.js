...
// Based on: https://docs.novita.ai/

class LLMService {
  constructor() {
    this.baseURL = 'https://api.novita.ai/openai/v1';
    this.models = {
      fast: 'meta-llama/llama-3.3-70b-instruct',  // Fast, cost-effective
      expert: 'deepseek/deepseek-r1'  // Advanced reasoning model
    };
    this.apiKey = null;
  ...
  getModelInfo() {
    if (this.mode === 'expert') {
      return { name: 'DeepSeek R1', description: 'Advanced reasoning model' };
    }
  ...
  async refinePrompt(originalPrompt, improvementType, improvementDetails) {
    ...
    if (!this.validateApiKey(this.apiKey)) {
      throw new Error('Invalid API key format. Please check your Novita AI API key.');
    }
